...
#include <xdp/common/nat_config.h>
...
struct {
    __uint(type, BPF_MAP_TYPE_ARRAY);
    __uint(key_size, sizeof(__u32));
    __uint(value_size, sizeof(struct vpn_global_config));
    __uint(max_entries, 1);
} vpn_config_map SEC(".maps");
...
static __always_inline int detect_and_log_snat(struct xdp_md *ctx,
    ...
    // 4. 读取 VPN 全局配置
    __u32 cfg_key = 0;
    struct vpn_global_config *vpn_cfg = bpf_map_lookup_elem(&vpn_config_map, &cfg_key);
    if (!vpn_cfg)
        return XDP_PASS;
    ...
    if (outer_src_port < vpn_cfg->port_start || outer_src_port > vpn_cfg->port_end) {
        bpf_trace_printk("Port %d not in range [%d,%d]\n",
                         sizeof("Port %d not in range [%d,%d]\n"),
                         outer_src_port, vpn_cfg->port_start, vpn_cfg->port_end);
        return XDP_PASS;
    }

    // 检查预留端口（展开循环）
    if (vpn_cfg->reserved_count > 0 && vpn_cfg->reserved_ports[0] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 1 && vpn_cfg->reserved_ports[1] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 2 && vpn_cfg->reserved_ports[2] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 3 && vpn_cfg->reserved_ports[3] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 4 && vpn_cfg->reserved_ports[4] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 5 && vpn_cfg->reserved_ports[5] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 6 && vpn_cfg->reserved_ports[6] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 7 && vpn_cfg->reserved_ports[7] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 8 && vpn_cfg->reserved_ports[8] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 9 && vpn_cfg->reserved_ports[9] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 10 && vpn_cfg->reserved_ports[10] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 11 && vpn_cfg->reserved_ports[11] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 12 && vpn_cfg->reserved_ports[12] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 13 && vpn_cfg->reserved_ports[13] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 14 && vpn_cfg->reserved_ports[14] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }
    if (vpn_cfg->reserved_count > 15 && vpn_cfg->reserved_ports[15] == outer_src_port)
        { bpf_trace_printk("Port %d reserved\n", sizeof("Port %d reserved\n"), outer_src_port); return XDP_PASS; }

    // 7. 选择公网 IP（通过 hash）
    __u32 ip_count = vpn_cfg->egress_ip_count;
    if (ip_count == 0) {
    ...
    __u32 outer_src_ip = vpn_cfg->egress_ips[ip_index];
    ...
    if (snat) {
        ...
        if (now - snat->timestamp > vpn_cfg->timeout_ns) {
    ...
    if (!snat) {
        ...
        struct snat_entry new_snat = {
            ...
            .egress_iface = vpn_cfg->egress_iface,
...
static __always_inline int detect_and_log_dnat(struct iphdr *ip,
    ...
    // 1. 检查目标 IP 是否是我们的公网 IP
    if (vpn_cfg->egress_ip_count > 0 && ip->daddr == vpn_cfg->egress_ips[0])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 1 && ip->daddr == vpn_cfg->egress_ips[1])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 2 && ip->daddr == vpn_cfg->egress_ips[2])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 3 && ip->daddr == vpn_cfg->egress_ips[3])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 4 && ip->daddr == vpn_cfg->egress_ips[4])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 5 && ip->daddr == vpn_cfg->egress_ips[5])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 6 && ip->daddr == vpn_cfg->egress_ips[6])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 7 && ip->daddr == vpn_cfg->egress_ips[7])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 8 && ip->daddr == vpn_cfg->egress_ips[8])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 9 && ip->daddr == vpn_cfg->egress_ips[9])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 10 && ip->daddr == vpn_cfg->egress_ips[10])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 11 && ip->daddr == vpn_cfg->egress_ips[11])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 12 && ip->daddr == vpn_cfg->egress_ips[12])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 13 && ip->daddr == vpn_cfg->egress_ips[13])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 14 && ip->daddr == vpn_cfg->egress_ips[14])
        { is_public_ip = 1; }
    if (vpn_cfg->egress_ip_count > 15 && ip->daddr == vpn_cfg->egress_ips[15])
        { is_public_ip = 1; }
    ...
    // 4. 检查超时
    __u64 now = bpf_ktime_get_ns();
    if (now - dnat->timestamp > vpn_cfg->timeout_ns) {
...
int xdp_gateway(struct xdp_md *ctx) {
    ...
    // NAT 逻辑（SNAT 和 DNAT）
    if (cfg_flag_enabled(cfg, CFG_FLAG_NAT_ENABLED)) {
        ...
        if (vpn_cfg) {
            ...
            if (is_public_ip) {
                return detect_and_log_dnat(ip, data_end);
            }
        }
    }
